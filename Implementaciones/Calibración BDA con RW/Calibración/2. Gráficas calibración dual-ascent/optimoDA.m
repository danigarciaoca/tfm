...
load('RL_dual_ascent_optimo.mat')
...
hold off
title(['DA (TD) -  Return per episode (always starting at s=2)'])
...
title('DA (TD) - \Sigmaq(s,a) for each episode')
...
title('DA (TD) - \Sigmav(s) for each episode')
...
title('DA (TD) - Mean-squared error of q(s,a) for each episode')
...
title('DA (TD) - Mean-squared error of v(s) for each episode')
