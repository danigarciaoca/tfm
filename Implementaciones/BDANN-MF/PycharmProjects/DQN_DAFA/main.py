...
# # # # AGENTE:
numExperiments = 2  # Número de experimentos de numRep*numEpisodes episodios1
...
BUFF_SIZE = 400 #4000
BATCH_SIZE = 100 #1000
trainingset = BufferCircular(BUFF_SIZE, 2) # 2 is de number of columns to store: x_train (1 col, states) and y_train (1 col, predicted bellman eq. r+gamma*prediction(NN1))
...
for exp in range(0, numExperiments):
    ...
    for k in range(0, numRep):
        ...
        for n in range(0, numEpisodes):  # This loop sets the update frequency of v (every numEpi episodes)
            ...
            # d_norm = d_opt_norm política óptima
            while not terminal:
                ...
                pred_nn1 = NN1_pred.predict(np.array([nextState]))
        ...
        # Theta parameters (weights) update (fit NN2)
        # Sum of both columns, along all rows from current index to the end
        sum_row = np.sum(trainingset.data[trainingset.index: BUFF_SIZE,:],0)
        # check if sum equals zero. It means that it is still the first time the buffer fills
        bool_sum_row = sum_row == 0
        # once the buffer is full...
        if not(bool_sum_row[0] and bool_sum_row[1]):
            # uniformly create a batch from trainingset
            if first_fit:
                first_fit = False
                batch = trainingset.data
                NN2_fit.fit(batch[:,0], batch[:,1], epochs=20, batch_size=128, verbose=2, callbacks=[tbCallback])
            else:
                ind = np.random.choice(BUFF_SIZE, BATCH_SIZE, replace=False)
                batch = trainingset.data[ind,:]
                #etapa warm, no hacer fit hasta que haya 4000
                NN2_fit.fit(batch[:,0], batch[:,1], epochs=20, batch_size=32, verbose=2, callbacks=[tbCallback])
            # Update parameters (weights) in NN1
        ...
        for i in range(0, totalStepsPerRep):
            ...
            if stepPerEpisode == maxNumStepsPerEpisode - 1:  # Si el estado siguiente es el terminal
                ...
                print("d normalizado: \n", d_norm.transpose())
                print("Experimento",exp,"repetición",k)
                episodeCountD += 1
