...
legend('max steps = 300','max steps = 500','max steps = 700')
...
numEpisodesAux = [50 100 200 300];
figure, hold on
for i = 1:size(numEpisodesAux,2)
    load([pathRes 'nExp=10,nEpi=' num2str(numEpisodesAux(i)) ',steps=500,eps=0.1.mat'])
    plot(1:size(Gmean,2), Gmean, 'LineWidth', 2)
end
hold off, xlim([0 300])
legend('numEpi=50','numEpi=100','numEpi=200','numEpi=300')
