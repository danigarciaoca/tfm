...
numRep = 400;%50; % Número de repeticiones de cada set de episodios
numEpisodes = 5;%20; % Número de episodios de cada repeticion
...
epsilon = 0.45;%0.15; % e-greedy value (entre 0.05 y 0.2) %con 0.4 tira bien para el cliff
alphaD = 3;%0.05; % Stepsize para la iteración de la variable dual d % 0.1 valor original
alphaTD = 0.2;%0.05; % Stepsize para la iteración de la variable primal v
    ...
    for k = 1:numRep
        ...
        for i = 1:totalStepsPerRep
            ...
            if any(isnan(d_norm))
                ...
                %d_orig(isnan(d_norm)) = abs(d_orig(isnan(d_norm)));
                d = d_orig;
                d(d<0)=0; % Projection of d over positives
                d_norm = getPolicyVectorFromD( d, env );
                % disp('nan!')
            ...
            if terminal || step == maxNumStepsPerEpisode-1 % Si el estado siguiente es el terminal
                ...
                [~, ~, G_eps0] = RL_core(env, d, v, 0, alphaTD, alphaD, numRep, numEpisodes, maxNumStepsPerEpisode, mult);
...
hold off, title(['BDA Return per episode (always starting at s=' ,num2str(env.initState), ')'])
