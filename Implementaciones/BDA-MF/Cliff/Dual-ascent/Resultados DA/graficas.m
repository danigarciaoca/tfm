...
alfaD = [3 2 1 0.8 0.6 0.5 0.4 0.2 0.1 0.08 0.05];
alfaTD = [0.9 0.8 0.6 0.5 0.4 0.2 0.1 0.08 0.05];
epsilon = [0.01 0.05 0.1 0.15 0.2 0.25 0.3 0.35 0.4 0.45 0.5 0.6 0.7 0.9 1];

save('Pruebas alfaD\Gmean_alphaD_nRep300.mat', 'Gmean_todos', 'mean_Gmean_eps0_todos', 'alfaD')
save('Pruebas alfaTD\Gmean_alphaTD_nRep300.mat', 'Gmean_todos', 'mean_Gmean_eps0_todos', 'alfaTD')
save('Pruebas epsilon\Gmean_epsilon_nRep300.mat', 'Gmean_todos', 'mean_Gmean_eps0_todos', 'epsilon')
%%
clear all, clc
...
legend('\alpha_D = 3','\alpha_D = 2','\alpha_D = 1','\alpha_D = 0.8','\alpha_D = 0.6'...
...
figure, plot([1:size(Gmean_todos,2)], Gmean_todos, 'LineWidth',2)
legend('\alpha_{TD} = 0.9','\alpha_{TD} = 0.8','\alpha_{TD} = 0.6'...
...
%%
clear all, clc
% load('Pruebas epsilon\Gmean_epsilon_nRep300.mat')
load('Pruebas epsilon\Gmean_epsilon.mat')
figure, plot([1:size(Gmean_todos,2)], Gmean_todos, 'LineWidth',2)
legend('\epsilon = 0.01', '\epsilon = 0.05', '\epsilon = 0.1', '\epsilon = 0.15', '\epsilon = 0.2'...
...
for i = 1:size(numEpisodesAux,2)
    load([pathRes 'nExp=20,nRep=50,nEpi=' num2str(numEpisodesAux(i)) ',alphaD=0.1,alphaTD=0.2,eps=0.1.mat'])
...
figure, hold on
for i = 1:size(numEpisodesAux,2)
    load([pathRes 'nExp=20,nRep=300,nEpi=' num2str(numEpisodesAux(i)) ',alphaD=0.1,alphaTD=0.2,eps=0.1.mat'])
    plot(1:size(Gmean,2), Gmean, 'LineWidth', 2)
end
hold off, xlim([0 1000])
legend('numEpi=1','numEpi=10','numEpi=20','numEpi=25','numEpi=30','numEpi=35','numEpi=40','numEpi=45','numEpi=50','numEpi=55','numEpi=60')
