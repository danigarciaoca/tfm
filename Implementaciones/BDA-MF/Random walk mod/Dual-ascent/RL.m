...
G_eps0_final = zeros(numExperiments, numRep*numEpisodes); % Return por episodio cuando epsilon = 0 y ya se ha convergido a la política óptima
G_eps0_each_epi = zeros(numExperiments, numRep*numEpisodes); % Return por episodio cuando epsilon = 0
epsilon = 0.4; % e-greedy value (entre 0.05 y 0.2)
    ...
    for k = 1:numRep
        ...
        for i = 1:totalStepsPerRep
            ...
            if sum(nextState == game.finalState) == 1 || stepPerEpisode == maxNumStepsPerEpisode-1 % Si el estado siguiente es el terminal
                ...
                [~, ~, G_eps0] = RL_core(game, d, v, 0, alphaTD, alphaD, 1, 1, maxNumStepsPerEpisode, mult1, mult2, init_state);
